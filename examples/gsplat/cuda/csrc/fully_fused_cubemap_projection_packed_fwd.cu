...
template <typename T>
__global__ void fully_fused_cubemap_projection_packed_fwd_kernel(
    const uint32_t C,
    const uint32_t N,
    ...
    if (block_cnts != nullptr) {
        ...
        //     printf("block_idx_y: %d, block_idx_x: %d, block_idx: %d, aggregate: %d, thread_data: %d\n", blockIdx.y, blockIdx.x, block_idx, aggregate, thread_data);
        // }
    } else {
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_cubemap_projection_packed_fwd_tensor(
    ...
    if (C && N) {
        ...
        fully_fused_cubemap_projection_packed_fwd_kernel<float>
        ...
        // printf("block_cnts shape: [%d]\n", block_cnts.size(0));
        // printf("block_cnts first 10 elements: ");
        // for (int i = 0; i < std::min(10, (int)block_cnts.size(0)); i++) {
        //     printf("%d ", block_cnts[i].item<int32_t>());
        // }
        // printf("\n");
        // printf("block_accum first 10 elements: ");
        // for (int i = 0; i < std::min(10, (int)block_accum.size(0)); i++) {
        //     printf("%d ", block_accum[i].item<int32_t>());
        // }
        // printf("\n");
        // printf("nnz: %d\n", nnz);
    } else {
