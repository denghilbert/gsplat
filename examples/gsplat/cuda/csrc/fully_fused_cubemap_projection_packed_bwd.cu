...
template <typename T>
__global__ void fully_fused_cubemap_projection_packed_bwd_kernel(
    // fwd inputs
    const uint32_t C,
    const uint32_t N,
    const uint32_t nnz,
...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
fully_fused_cubemap_projection_packed_bwd_tensor(
    // fwd inputs
    ...
    if (nnz) {
        fully_fused_cubemap_projection_packed_bwd_kernel<float>
